...
number_of_applicant = 800 # S券交換希望者の人数
...
while i <= number_of_applicant:
    ...
    # 日付:int(l[i-1][0]) が 1~18:9/21, 19~36:9/22, 37~54:9/24
    if 1 <= int(l[i-1][0]) <= 18 :
        g.write('9/21 ')
    elif 19 <= int(l[i-1][0]) <= 36:
        g.write('9/22 ')
    else:
        g.write('9/24 ')
    # 時:int(l[i-1][0]) が (1~6 or 19~24 or 37~42):14, (7~12 or 25~30 or 43~48):15, (13~18 or 31~36 or 49~54):16
    if (1 <= int(l[i-1][0]) <= 6) or (19 <= int(l[i-1][0]) <= 24) or (37 <= int(l[i-1][0]) <= 42):
        g.write('14:')
    elif (7 <= int(l[i-1][0]) <= 12) or (25 <= int(l[i-1][0]) <= 30) or (43 <= int(l[i-1][0]) <= 48):
        g.write('15:')
    else:
        g.write('16:')
    # 分:int(l[i-1][0]) が (((6で割った余り) + 5) を 6 で割った余り)×10
    g.write(str((( (int(l[i-1][0])%6) +5)%6)*10) + ',' + '->' + ',')
    
    # applicant No.i の交換が成立した場合, 交換後のスロットを書き込む
    if allCycle[i-1][1] != -1:
        # 日付:int(l[(allCycle[i-1][1])-1][0]) が 1~18:9/21, 19~36:9/22, 37~54:9/24
        if 1 <= int(l[(allCycle[i-1][1])-1][0]) <= 18 :
            g.write('9/21 ')
        elif 19 <= int(l[(allCycle[i-1][1])-1][0]) <= 36:
            g.write('9/22 ')
        else:
            g.write('9/24 ')
        # 時:int(l[(allCycle[i-1][1])-1][0]) が (1~6 or 19~24 or 37~42):14, (7~12 or 25~30 or 43~48):15, (13~18 or 31~36 or 49~54):16
        if (1 <= int(l[(allCycle[i-1][1])-1][0]) <= 6) or (19 <= int(l[(allCycle[i-1][1])-1][0]) <= 24) or (37 <= int(l[(allCycle[i-1][1])-1][0]) <= 42):
            g.write('14:')
        elif (7 <= int(l[(allCycle[i-1][1])-1][0]) <= 12) or (25 <= int(l[(allCycle[i-1][1])-1][0]) <= 30) or (43 <= int(l[(allCycle[i-1][1])-1][0]) <= 48):
            g.write('15:')
        else:
            g.write('16:')
        # 分:int(l[(allCycle[i-1][1])-1][0]) が (((6で割った余り) + 5) を 6 で割った余り)×10
        g.write(str((( (int(l[(allCycle[i-1][1])-1][0])%6) +5)%6)*10))
